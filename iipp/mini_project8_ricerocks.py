...
def group_collide(group, other_object):
    ...
    for obj in set(group):
        if obj.collide(other_object):
           group.remove(obj)
           an_explosion = Sprite(obj.pos, (0, 0), 0, 0, explosion_image, explosion_info, explosion_sound)
...
def draw(canvas):
    ...
    if not started:
        canvas.draw_image(splash_image, splash_info.center, 
                          splash_info.size, [WIDTH / 2, HEIGHT / 2], 
                          splash_info.size)
